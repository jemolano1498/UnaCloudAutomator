...
public abstract class VirtualBox extends Platform {
     ...
     */
    @Override
    public void stopExecution(String imageName){
		LocalProcessExecutor.executeCommandOutput(getExecutablePath(), "controlvm", imageName, "poweroff");
        sleep(2000);
    }
     ...
     */
    @Override
	public void unregisterImage(String image){
        LocalProcessExecutor.executeCommandOutput(getExecutablePath(), "unregistervm", image);
        sleep(15000);
    }
     ...
     */
    @Override
	public void startExecution(String imageName) throws PlatformOperationException {
//		setPriority(image);
        String h = LocalProcessExecutor.executeCommandOutput(getExecutablePath(), "startvm", imageName, "--type", "headless");
        if (h.contains(ERROR_MESSAGE)) {
            throw new PlatformOperationException(h.length() < 100 ? h : h.substring(0, 100));
        }
        sleep(30000);
...
//		}
     ...
     */
    @Override
    public void executeCommandOnExecution(String image,String command, String... args) throws PlatformOperationException {
        List<String> com = new ArrayList<>();
        Collections.addAll(com, createExecutionCommand(getExecutablePath(), image, command, "root", "root"));
     ...
     */
    @Override
    public void copyFileOnExecution(String image, String destinationRoute, File sourceFile) throws PlatformOperationException {
       	String h = LocalProcessExecutor.executeCommandOutput(createCopyToCommand(getExecutablePath(), image, sourceFile.getAbsolutePath(), destinationRoute, "root", "root"));
    	if (h.contains(ERROR_MESSAGE)) {
            throw new PlatformOperationException(h.length() < 100 ? h : h.substring(0, 100));
        }
        sleep(10000);
